...
const Dashboard = () => {
    ...
    const boardsQuery = user?.uid && ref.collectionGroup("boards").where("boardAssigneesEmails", "array-contains", user.email)
    const [boards] = useCollectionData(boardsQuery, { idField: "id" });

    //To update component with live owned projects and assigned projects
    useEffect(() =>  {
    }, [user, assigneeProjects , boards ])
    return (
        <>
            <SideBar assigneeProjects={assigneeProjects} boards={boards} setCurrentProject={setCurrentProject} setCurrentBoard={setCurrentBoard} />
            <TaskDisplay currentProject={currentProject} currentBoard={currentBoard}/>
