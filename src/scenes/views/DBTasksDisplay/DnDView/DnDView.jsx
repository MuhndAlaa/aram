                            ...
                            (provided, snapshot) => (
                                ...
                                <div ref={provided.innerRef}  {...provided.droppableProps}>
                                    {col.items?.map((task, taskIndex) => (
                                        <Draggable className={`${currentView}-task`} draggableId={task.id} key={task.id} index={taskIndex}>
                                            {
                                                // Div below is the div of tasks (Can replace it with a task component)
                                                (provided, snapshot) => (
                                                    <div className={`board-card ${task.status}-task`} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                                        {(currentView === 'board') && <BoardCard task={task}/>}
                                                        {(currentView === 'list') && <div>add a different component </div>}
                                                    </div>
                                                )
                                            }
                                        </Draggable>
